...
## このプロジェクト固有: フロントエンド（ページ）テスト

playground のページ（`src/components/pages`）を追加・変更したら、Rust の `cargo test` だけで
なく **Vitest コンポーネントテストを `src/test/` に追加**する（WASM コアのテストだけで終えない）。

| 基準 | 判定 |
|------|------|
| 新規・変更したページに Vitest コンポーネントテストがない | REJECT |
| WASM ロードを実 `.wasm` で検証せず、exports を全面モックしている | 警告。実ロジックを UI 越しに検証する |
| canvas/rAF ページで getContext・rAF をスタブし状態遷移を決定的に検証している | OK |

- WASM ロードは実 `.wasm` バイト列で `fetch` を差し替えて検証する（`src/test/wasm.ts` の
  `mockWasmFetch`/`mockWasmFetchFailure` を使い、モックの振る舞いでなく実 Rust ロジックを
  UI 越しに検証する）。
- 最低限の観点: WASM ロード→ready 遷移、主要操作（生成/開始等）、localStorage 永続、
  エラー状態の表示。
- エラーパスで意図的に `console.error` する場合は spy で抑止し、テスト出力をクリーンに保つ。
- 参考実装: `src/test/{JiyuGacha,FreeHaiku,LeapToFreedom}.test.tsx`、`src/test/wasm.ts`。
