...
## このプロジェクト固有ルール: Rust → WASM ページ

playground（ナナオのあそびば）に Rust→WASM の作品ページを追加・改修する際は、既存の
gacha / haiku / leap と方式を揃える。

- **グローバル状態に `unsafe` を使わない。** WASM の FFI でグローバル状態（PRNG・ゲーム
  世界・スコアバッファ等）を保持する場合は `thread_local! + RefCell` を使う
  （`jiyu-gacha`/`free-haiku`/`leap-to-freedom` と同じ）。`static mut` + `addr_of!` は
  使わない。単一スレッド WASM 前提でも `unsafe` を増やさない。
- **WASM ロードを各ページで再実装しない。** `src/hooks/useWasm.ts`（`{exports, status}` を
  返す）を使う。`fetch → arrayBuffer → WebAssembly.instantiate(buf, {})` をページに
  直接書かない。
- **ページコンポーネントは薄く保つ（アトミックデザイン）。** データ定義・純粋ロジック・
  WASM 型は別モジュールへ分離し、UI は atoms（`src/components/atoms`）/ molecules
  （`src/components/molecules`、共有部品は prefix/className でスコープCSSに対応）/
  organisms（各ページフォルダに co-locate）に分解する。pages は組み立てに徹する。
- **WASM は外部 import なし**を維持する（`WebAssembly.instantiate(buf, {})` 可能）。Rust 側は
  ヒープ/Vec/format!/std::collections を避け、固定長配列＋数値演算、`panic = "abort"`。
- **CSS はページ単位スコープ**（`jg-`/`fh-`/`ltf-` のような接頭辞）。共有 UI 部品には
  className/prefix を渡してスコープを壊さない。
- **共通ユーティリティを再実装しない。** localStorage I/O は `src/lib/storage.ts`、シード
  生成は `src/lib/seed.ts` を使う。
- **Rust ソースとビルドスクリプトを必ずコミット**し、`npm run build:wasm` に新クレートの
  `build.sh` を追加して再生成可能にする。
